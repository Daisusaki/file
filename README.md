        ...
        if current.src < before.src
            ...
            if valid
                ...
                if remove
                    ...
                    linefill.new(d.upln.get(0), d.upln.get(1), color = color.new(cdn, 40))
                    upbin.clear()
                    broken := false
        ...
        if current.src > before.src
            ...
            if valid
                ...
                if remove
                    ...
                    linefill.new(d.dnln.get(0), d.dnln.get(1), color = color.new(cup, 40))
                    dnbin.clear()
                    broken := false
...
period = input.timeframe(title='Primary TL Period', defval='', group = GROUP_PIVOT)
period1 = input.timeframe(title='Secondary TL Period', defval='60', group = GROUP_PIVOT)
// period1 = ''
...
plotshape(showBSP ? TDUp==9?true:na : na,style=shape.triangledown, color=color.red, text="❌",textcolor=color.red, location=location.abovebar)
TS = 0
...
// Backtest Drawing
//************************************************//
//var BTM = "████ Visual Backtest Module ████"
//drawTester = input.bool(false, "Backtest Summary", group = BTM)

//TSP_price = goLong ? close * (1 - 0.1) : close * (1 + 0.1)
//EPSLTP = input.bool(false, "Plot Visual Backtest Module", group = "")
//plot_EP = plot(EPSLTP ? entry_price : na, title="Entry Level", color=color.blue,  linewidth=1, style=plot.style_linebr)
//plot_SL = plot(EPSLTP ? SL_price : na, title="Stop Loss",   color=color.red,   linewidth=2, style=plot.style_linebr)
//plot_TP = plot(EPSLTP ? TP_price : na, title="Take Profit", color=color.green, linewidth=2, style=plot.style_linebr)

//fill(plot_TP, plot_EP, title="TP Background", color=color.new(color.green, 75))
//fill(plot_SL, plot_EP, title="SL Background", color=color.new(color.red, 75))

//plotshape(goLong, title = "Long", color = color.yellow, textcolor = color.yellow, text = "↑L", style = shape.triangleup, location = location.belowbar, size = size.small)
//plotshape(goShort, title = "Short", color = color.fuchsia, textcolor = color.fuchsia, text = "↓S", style = shape.triangledown, location = location.abovebar, size = size.small)
...
// Prepare stats table
//var table testTable = table.new(position.bottom_left, 5, 2, border_width=1)
...
//                              /____/                  
// ==========================================================================
// @description The simplest version of the indicator automatically draws trendLine on your charts, with build-in functions only.
// @version 1.0
// @date 2022/6/16
// @author feibilanceon
...
if htfhindex != htfhindex[1] and PrimaryTrendline
    ...
    htflineHigh := line.new(htfhindex[1], htfhprice[1], htfhindex, htfhprice, extend=extend.none, color=color.new(color.red,25), width=4)
...
if ltfhindex != ltfhindex[1] and SecondaryTrendline
    ...
    ltflineHigh := line.new(ltfhindex[1], ltfhprice[1], ltfhindex, ltfhprice, extend=extend.none, color=color.new(#ff4b78, 25), width=2)
...
if htflindex != htflindex[1] and PrimaryTrendline
    ...
    htflineLow := line.new(htflindex[1], htflprice[1], htflindex, htflprice, extend=extend.none,color=color.new(color.green,25), width=4)
...
if ltflindex != ltflindex[1] and SecondaryTrendline
    ...
    ltflineLow := line.new(ltflindex[1], ltflprice[1], ltflindex, ltflprice, extend=extend.none,color=color.new(#008f6b, 10), width=2)


// © TheeHoganator 

// Updated Picture for the indicator
//indicator(title="Ultimate Indicator", shorttitle="Ultimate", overlay=true, max_labels_count = 201, precision = 3) // precision = 2 

//------------------------------------------------------------------------ // ***** Inputs ***** \\ ------------------------------------------------------------------------\\

//source = close //input(title="Indicator", defval=close)


    // Trend Settings
//trend_length = 50 //input.int(48, minval=1,   title="Trend EMA", inline = "01")

//----------------------------------------------------------------------- // ***** Screener ***** \\ ------------------------------------------------------------------------\\


//bScreener = input.bool(false, "████ Screener ████")
//showSignalTable = input.bool(false , title="Signal Tabel", tooltip="Checkmarks provide confirmation",  group = "Screener")
//currentEMA = ta.ema(close, trend_length)
...
//tableTransp1 = 30
//tableTransp2 = tableTransp1 >= 20 ? (tableTransp1 - 20) : tableTransp1
...
//plot(volume, style=plot.style_columns, title="Volume Histogram", color=color.blue, linewidth=5)

// Plot buying and selling volume
//averageVolume = ta.sma(volume, 50)
//totalVolume = volume + averageVolume
//buyerPercentage = buyingVolume / totalVolume * 100
//sellerPercentage = sellingVolume / totalVolume * 100

//Buyer = "BV-" + str.tostring(math.round(buyerPercentage, 0)) + "%"
//Seller = "SV-" + str.tostring(math.round(sellerPercentage, 0)) + "%"
//Final = buyingVolume > sellingVolume ? "Bull" : "Bear"

//BVC = buyingVolume > averageVolume ? buyingVolume > sellingVolume ? color.new(color.green, tableTransp1) : color.new(color.red, tableTransp1) : color.new(color.gray, tableTransp1)
//SVC = sellingVolume > averageVolume ? buyingVolume < sellingVolume ? color.new(color.green, tableTransp1) : color.new(color.red, tableTransp1) : color.new(color.gray, tableTransp1)
//VolCol = volume > averageVolume ? buyingVolume > sellingVolume ? color.new(color.green, tableTransp1) : color.new(color.red, tableTransp1) : color.new(color.gray, tableTransp1)


//ema1m    = request.security(syminfo.tickerid, tf_1m, currentEMA)
//Trend1m  = ema1m
//col_1m   = close > Trend1m ? color.new(color.green, tableTransp1) : color.new(color.red, tableTransp1)
//text_1m  = close > Trend1m ? "Up"        : "Down"

//ema5m    = gethtf(tf_5m, currentEMA)
//Trend5m  = ema5m
//col_5m   = close > Trend5m ? color.new(color.green, tableTransp1) : color.new(color.red, tableTransp1)
//text_5m  = close > Trend5m ? "Up"        : "Down"

//ema15m   = gethtf(tf_15m, currentEMA)
//Trend15m = ema15m
//col_15m  = close > Trend15m ? color.new(color.green, tableTransp1) : color.new(color.red, tableTransp1)
//text_15m = close > Trend15m ? "Up"        : "Down"

//ema60m   = gethtf(tf_60m, currentEMA)
//Trend60m = ema60m
//col_60m  = close > Trend60m ? color.new(color.green, tableTransp1) : color.new(color.red, tableTransp1)
//text_60m = close > Trend60m ? "Up"        : "Down"

//ema240m   = gethtf(tf_240m, currentEMA)
//Trend240m = ema240m
//col_240m  = close > Trend240m ? color.new(color.green, tableTransp1) : color.new(color.red, tableTransp1)
//text_240m = close > Trend240m ? "Up"        : "Down"


//ColShortTrend = Trend1 == 1 ? color.new(color.green, tableTransp1) : color.new(color.red, tableTransp1)
//ColMidTrend = Trend2 == 1 ? color.new(color.green, tableTransp1) : color.new(color.red, tableTransp1)
//ColLongTrend = Trend3 == 1 ? color.new(color.green, tableTransp1) : color.new(color.red, tableTransp1)

//FinalTrend = ColShortTrend == color.new(color.green, tableTransp1) and ColMidTrend == color.new(color.green, tableTransp1) and ColLongTrend == color.new(color.green, tableTransp1) ? "Buy" : ColShortTrend == color.new(color.red, tableTransp1) and ColMidTrend == color.new(color.red, tableTransp1) and ColLongTrend == color.new(color.red, tableTransp1) ? "Sell" : "Wait"
//ColFinalTrend = FinalTrend == "Buy" ? color.new(color.green, tableTransp1) : FinalTrend == "Sell" ? color.new(color.red, tableTransp1) : color.new(color.gray, tableTransp1)

//alertcondition(Trend1 == 1, title='.Supertrend Buy 🟢', message='Supertrend Buy 🟢')
//alertcondition(Trend1 == 1, title='.Supertrend Sell 🔴', message='Supertrend Sell 🔴')
...
//var tbl = table.new(position.bottom_right, 3, 4, frame_color=#151715, frame_width=1, border_width=2, border_color=color.new(color.white, 100))        //table.new(position.top_right, 6, 41, frame_color=#151715, frame_width=1, border_width=2, border_color=color.new(color.white, 100))

//if barstate.isconfirmed and bScreener and time >= starttime//and bFlip

//    table.cell(tbl, 0, 0, "5m",  text_halign = text.align_center, bgcolor = col_5m,  text_color = color.white, text_size = size.normal)
//    table.cell(tbl, 1, 0, "15m", text_halign = text.align_center, bgcolor = col_15m, text_color = color.white, text_size = size.normal)
//    table.cell(tbl, 2, 0, "1H", text_halign = text.align_center, bgcolor = col_60m, text_color = color.white, text_size = size.normal)
//    table.cell(tbl, 0, 1, "ST",  text_halign = text.align_center, bgcolor = ColShortTrend,  text_color = color.white, text_size = size.normal)
//    table.cell(tbl, 1, 1, "MT" , text_halign = text.align_center, bgcolor = ColMidTrend, text_color = color.white, text_size = size.normal)
//    table.cell(tbl, 2, 1, "LT", text_halign = text.align_center, bgcolor = ColLongTrend, text_color = color.white, text_size = size.normal)
//    table.cell(tbl, 0, 2, currentTrend, bgcolor = currentTrend == 'Bull' ? color.new(color.green, tableTransp1) : color.new(color.red, tableTransp1), text_color=color.white, text_size = size.normal)
//    table.cell(tbl, 1, 2, reversalProbValue, bgcolor = currentTrend == 'Bull' ? color.new(color.green, tableTransp1) : color.new(color.red, tableTransp1), text_color=color.white, text_size = size.normal)
//    table.cell(tbl, 2, 2, FinalTrend, bgcolor = color.new(ColFinalTrend, tableTransp1), text_color=color.white, text_size = size.normal)
//    table.cell(tbl, 0, 3, str.tostring(Seller), text_halign = text.align_center, bgcolor = SVC, text_color = color.white, text_size = size.normal)
//    table.cell(tbl, 1, 3, str.tostring(Buyer), text_halign = text.align_center, bgcolor = BVC, text_color = color.white, text_size = size.normal)
//    table.cell(tbl, 2, 3, Final, bgcolor = VolCol, text_color=color.white, text_size = size.normal)

//var tbl2 = table.new(position.middle_right, 2, 6, frame_color=#151715, frame_width=1, border_width=2, border_color=color.new(color.white, 100))        //table.new(position.top_right, 6, 41, frame_color=#151715, frame_width=1, border_width=2, border_color=color.new(color.white, 100))

//stat = label.new(time, close, text = "Signal = " + str.tostring(signal) + "\n" + "Profit = " + str.tostring(math.round(profit, 2)) + "%" + "\n" + "Premium Gain = " + str.tostring(math.round(PremiumDiff, 2)) + "\n" + "Entry Price - " + str.tostring(math.round(entry_price, 2)) + "\n" + "SL Price - " + str.tostring(math.round(SL_price, 2)) + "\n" + "TP Price - " + str.tostring(math.round(TP_price, 2)), color = color.white, xloc=xloc.bar_time, yloc=yloc.abovebar, size = size.large, style = label.style_label_lower_left)

//if barstate.isconfirmed and showSignalTable and time >= starttime//and bFlip

//    table.cell(tbl2, 0, 0, "Signal",  text_halign = text.align_center, bgcolor = color.black,  text_color = color.white, text_size = size.normal)
//    table.cell(tbl2, 0, 1, "Profit", text_halign = text.align_center, bgcolor = color.black, text_color = color.white, text_size = size.normal)
//    table.cell(tbl2, 0, 2, "Gain", text_halign = text.align_center, bgcolor = color.black, text_color = color.white, text_size = size.normal)
//    table.cell(tbl2, 0, 3, "Entry",  text_halign = text.align_center, bgcolor = color.black,  text_color = color.white, text_size = size.normal)
//    table.cell(tbl2, 0, 4, "SL" , text_halign = text.align_center, bgcolor = color.black, text_color = color.white, text_size = size.normal)
//    table.cell(tbl2, 0, 5, "TP", text_halign = text.align_center, bgcolor = color.black, text_color = color.white, text_size = size.normal)
//    table.cell(tbl2, 1, 0, str.tostring(signal), bgcolor = color.black, text_color=color.white, text_size = size.normal)
//    table.cell(tbl2, 1, 1, str.tostring(math.round(profit, 2)) + "%", bgcolor = color.black, text_color=color.white, text_size = size.normal)
//    table.cell(tbl2, 1, 2, str.tostring(math.round(PremiumDiff, 2)), bgcolor = color.black, text_color=color.white, text_size = size.normal)
//    table.cell(tbl2, 1, 3, str.tostring(math.round(entry_price, 2)), text_halign = text.align_center, bgcolor = color.black, text_color = color.white, text_size = size.normal)
//    table.cell(tbl2, 1, 4, str.tostring(math.round(SL_price, 2)), text_halign = text.align_center, bgcolor = color.black, text_color = color.white, text_size = size.normal)
//    table.cell(tbl2, 1, 5, str.tostring(math.round(TP_price, 2)), bgcolor = color.black, text_color=color.white, text_size = size.normal)
...
RSIi_obThreshold = input.int(70, 'Overbought', minval=50, maxval=100, inline='Thresh1', group='RSI Threshold Settings')
RSIi_bullZone    = input.int(60, 'Bull Zone' , minval=50, maxval=65 , inline='Thresh1', group='RSI Threshold Settings')
RSIi_bearZone    = input.int(40, 'Bear Zone' , minval=35, maxval=50 , inline='Thresh2', group='RSI Threshold Settings')
RSIi_osThreshold = input.int(30, ' Oversold' , minval=1 , maxval=50 , inline='Thresh2', group='RSI Threshold Settings')
...
oscHight        = 11 - input.int(10, ' Hight' , minval = 3, maxval = 30        , inline='VOL'  , group = group_rsi_osc)
...
//# * ══════════════════════════════════════════════════════════════════════════════════════════════
//# *
//# * Study       : ADX Support & Resistance Zones
//# * Author      : © dgtrd
//# *
//# * Revision History
//# *  Release    : Mar 09, 2021
//# *  Update     : Mar 13, 2021 : Alert functionality added 
//# *  Update     : Mar 22, 2021 : Source selection extended by adding On Balance Volume (OBV) option
//# *  Update     : Mar 08, 2022 : added ADX oscillator overlay display 
//# *
//# * ══════════════════════════════════════════════════════════════════════════════════════════════
// ══════════════════════════════════════════════════════════════════════════════════════════════════ //
...
ADXi_obThreshold = input.int(70, 'Overbought', minval=50, maxval=100, inline='Thresh1', group='ADX Threshold Settings')
ADXi_bullZone    = input.int(60, 'Bull Zone' , minval=50, maxval=65 , inline='Thresh1', group='ADX Threshold Settings')
ADXi_bearZone    = input.int(40, 'Bear Zone' , minval=35, maxval=50 , inline='Thresh2', group='ADX Threshold Settings')
ADXi_osThreshold = input.int(30, ' Oversold' , minval=1 , maxval=50 , inline='Thresh2', group='ADX Threshold Settings')
...
ADXoscHight        = 11 - input.int(10, ' Hight' , minval = 3, maxval = 30        , inline='VOL'  , group = group_ADX_osc)
...
var string KRS = "KRO - Settings", var string KRU = "KRO - UI"
...
wave = plot(ROpoints ? Wave : na, "Signal Wave", BCol ? color(na) : Wave > Wave[1] ? UpCol : DnCol, 3)
...
//
// Credits:
//      Pivot Points High Low Multi Time Frame by LonesomeTheBlue: https://www.tradingview.com/script/w2cENzrs-Pivot-Points-High-Low-Multi-Time-Frame/
//
// v001
//

//indicator("Consolidation Spotter Multi Time Frame", overlay = true, max_lines_count = 500, max_labels_count = 500, max_boxes_count = 500)

// ---- Setup ----
var string CRF = "████ CONSOLIDATION RANGE ████"
pptimeframe = input.timeframe(defval = '', title = "Time Frame", group = CRF)
leftBars  = input.int(defval = 2, title = "Left Bars", minval = 1, group = CRF)
rightBars = input.int(defval = 2, title = "Right Bars", minval = 1, group = CRF)
pivotsBack = input.int(defval = 1, title = "Pivots Back (High/Low)", minval = 1, group = CRF)
extendBoxLeft = input.int(defval = 6, title = "Extend Box", minval = 3, maxval = 100, group = CRF)
showCB = input.bool(defval = false, title = "Show Consolidation Box", group = CRF)
showMid = input.bool(defval = true, title = "Show Box Mid", group = CRF)
showPivots = input.bool(defval = false, title = "Show Pivots", group = CRF)
showTfLabel = input.bool(defval = true, title = "Show Labels", group = CRF)
boxlinestyle = input.string(defval = "Solid", title = "Border Style/Width", options = ["Solid", "Dotted", "Dashed"], inline = "boxbs", group = "Consolidation Box")
boxlinewidth = input.int(defval = 1, title = "", minval = 1, maxval = 10, inline = "boxbs", group = "Consolidation Box")
boxmidlinestyle = input.string(defval = "Dashed", title = "Mid Style/Width", options = ["Solid", "Dotted", "Dashed"], inline = "boxmidbs", group = "Consolidation Box")
boxmidlinewidth = input.int(defval = 1, title = "", minval = 1, maxval = 10, inline = "boxmidbs", group = "Consolidation Box")
boxtheme = input.string(defval = "One", title = "Colors", options = ["One", "Three", "Five"], inline = "self", group = "Consolidation Box")
boxbreakupcol = input.color(defval = color.lime, title = "Break Up Color/Fill", inline = "bcup", group = "Consolidation Box")
boxfillbreakupcol = input.color(defval = color.new(color.lime, 95), title = "", inline = "bcup", group = "Consolidation Box")
boxmidupcol = input.color(defval = color.teal, title = "Mid Up Color/Fill", inline = "bcmup", group = "Consolidation Box")
boxfillmidupcol = input.color(defval = color.new(color.teal, 95), title = "", inline = "bcmup", group = "Consolidation Box")
boxcol = input.color(defval = color.silver, title = "Neutral Color/Fill", inline = "bcn", group = "Consolidation Box")
boxfillcol = input.color(defval = color.new(color.silver, 95), title = "", inline = "bcn", group = "Consolidation Box")
boxmiddncol = input.color(defval = color.maroon, title = "Mid Down Color/Fill", inline = "bcmdn", group = "Consolidation Box")
boxfillmiddncol = input.color(defval = color.new(color.maroon, 95), title = "", inline = "bcmdn", group = "Consolidation Box")
boxbreakdncol = input.color(defval = color.red, title = "Break Down Color/Fill", inline = "bcdn", group = "Consolidation Box")
boxfillbreakdncol = input.color(defval = color.new(color.red, 95), title = "", inline = "bcdn", group = "Consolidation Box")
phlinecol = input.color(defval = color.blue, title = "Pivot High/Low Line Colors", inline = "lc", group = "Pivot Points")
pllinecol = input.color(defval = color.red, title = "", inline = "lc", group = "Pivot Points")
phbgcol = input.color(defval = color.new(#131722, 20), title = "Pivot High BG/Text Colors", inline = "ph", group = "Pivot Points")
phtextcol = input.color(defval = color.blue, title = "", inline = "ph", group = "Pivot Points")
plbgcol = input.color(defval = color.new(#131722, 20), title = "Pivot Low BG/Text Colors", inline = "pl", group = "Pivot Points")
pltextcol = input.color(defval = color.red, title = "", inline = "pl", group = "Pivot Points")
textSizeLabel = input.string("Normal", title = "Label Size", options = ["Normal", "Small", "Tiny"], group = "Labels")
// ---- Setup ----
...
//indicator(title='Directional Strength Panel', shorttitle='Directional Strength Panel', overlay=true)
...
in_theme = input.string('Dark', title='Chart Theme', group=grp1, options=['Light', 'Dark'])
in_size = input.string('Small', title='Table Size', group=grp1, options=['Small', 'Large'])
// -------
ma_typ = input.string('ALMA', title='MA Type', group=grp3, options=['ALMA', 'EMA', 'SuperSmoother'])
ma_len = input.int(20, title='MA Length', group=grp3)
offset = input.float(0.6, title='  Offset', group=grp3, tooltip=tips2)
sigma = input.float(6, title='  Sigma', group=grp3, tooltip=tips2)
// -------
ticker_1 = 'BYBIT:ETHUSDT.P' //input.symbol('TVC:USOIL', title='Ticker 1', group=grp2, inline='Ticker 1')
ticker_2 = 'BYBIT:BTCUSDT.P' //input.symbol('OANDA:XAUUSD', title='Ticker 2', group=grp2, inline='Ticker 2')

if barstate.isfirst  //Index
    array.push(lbl_list, 'ETH') //input.string('USOIL', title='', group=grp2, inline='Ticker 1', tooltip=tips))  //0
    array.push(lbl_list, 'BTC') //input.string('XAUUSD', title='', group=grp2, inline='Ticker 2', tooltip=tips))  //1
// ---------------------------
// Variables
// ---------------------------
var t_position = position.middle_right
var col_text = in_theme == 'Dark' ? color.rgb(255, 255, 255) : color.rgb(0, 0, 0)
    ...
    for i = 0 to 6
        ...
        if input_labels
            ...
            if labelArray.size() < 7
                labelArray.push(label.new(na, na, na, style = label.style_flag))
...
smaColor = input(color.rgb(253, 1, 1), title="Color for SMA")
emaColor = input(color.rgb(57, 202, 0), title="Color for EMA")
slMultiplier = input(2.5, title="ATR Multiplier for Stop Loss")
tpMultiplier = input(5.0, title="ATR Multiplier for Take Profit")
slColor = input(color.rgb(253, 1, 1), title="Color for Stop Loss Line")
tpColor = input(color.rgb(57, 202, 0), title="Color for Take Profit Line")
entryColor = input(color.rgb(54, 104, 243), title="Color for Entry Line")

// Calculate SMA and EMA
sma = ta.sma(close, smaLength)
ema = ta.ema(close, emaLength)

// Define crossover and crossunder
buySignal = ta.crossover(ema, sma)
sellSignal = ta.crossunder(ema, sma)

// Calculate ATR, SL and TP

slLevelLong = close - slMultiplier*atr
tpLevelLong = close + tpMultiplier*atr
slLevelShort = close + slMultiplier*atr
tpLevelShort = close - tpMultiplier*atr

// Plot the SMA and EMA
...
label_offset =  input.int(20, 'Label offsets', group = "SETTINGS")
...
if buySignal
    ...
    slLine := line.new(x1=bar_index, y1=slLevelLong, x2=bar_index+label_offset, y2=slLevelLong, color=slColor, width = 2, extend = extend.none)
    tpLine := line.new(x1=bar_index, y1=tpLevelLong, x2=bar_index+label_offset, y2=tpLevelLong, color=tpColor, width = 2, extend = extend.none)
    entryLine := line.new(x1=bar_index, y1=close, x2=bar_index+label_offset, y2=close, color=entryColor, width = 2, extend = extend.none)
    
    // Modifications : Décalage de 2 bars à droite, style label_left pour placer l'étiquette après le prix
    slLabel := label.new(x=bar_index + label_offset, y=slLevelLong, text="Stop Loss", color=slColor, style=label.style_label_left, textcolor=color.white, xloc=xloc.bar_index, yloc=yloc.price, textalign=text.align_right)
    tpLabel := label.new(x=bar_index + label_offset, y=tpLevelLong, text="Take Profit", color=tpColor, style=label.style_label_left, textcolor=color.white, xloc=xloc.bar_index, yloc=yloc.price, textalign=text.align_right)
    entryLabel := label.new(x=bar_index + label_offset, y=close, text="Entry", color=entryColor, style=label.style_label_left, textcolor=color.white, xloc=xloc.bar_index, yloc=yloc.price, textalign=text.align_right)
    ...
    slLine := line.new(x1=bar_index, y1=slLevelShort, x2=bar_index+label_offset, y2=slLevelShort, color=slColor, width = 2, extend = extend.none)
    tpLine := line.new(x1=bar_index, y1=tpLevelShort, x2=bar_index+label_offset, y2=tpLevelShort, color=tpColor, width = 2, extend = extend.none)
    entryLine := line.new(x1=bar_index, y1=close, x2=bar_index+label_offset, y2=close, color=entryColor, width = 2, extend = extend.none)
    
    // Modifications : Décalage de 2 bars à droite, style label_left pour placer l'étiquette après le prix
    slLabel := label.new(x=bar_index + label_offset, y=slLevelShort, text="Stop Loss", color=slColor, style=label.style_label_left, textcolor=color.white, xloc=xloc.bar_index, yloc=yloc.price, textalign=text.align_right)
    tpLabel := label.new(x=bar_index + label_offset, y=tpLevelShort, text="Take Profit", color=tpColor, style=label.style_label_left, textcolor=color.white, xloc=xloc.bar_index, yloc=yloc.price, textalign=text.align_right)
    entryLabel := label.new(x=bar_index + label_offset, y=close, text="Entry", color=entryColor, style=label.style_label_left, textcolor=color.white, xloc=xloc.bar_index, yloc=yloc.price, textalign=text.align_right)
    ...
    slLabel := label.new(x=bar_index + label_offset, y=slLevelShort, text="Stop Loss", color=color.new(#0332b4, 0), style = label.style_label_right, textcolor=color.white, xloc=xloc.bar_index, yloc=yloc.price, textalign=text.align_right)
    tpLabel := label.new(x=bar_index + label_offset, y=tpLevelShort, text="Take Profit", color=color.new(#0332b4, 0), style = label.style_label_right, textcolor=color.white, xloc=xloc.bar_index, yloc=yloc.price, textalign=text.align_right)
    entryLabel := label.new(x=bar_index + label_offset, y=close, text="Entry", color=color.new(#0332b4, 0), style = label.style_label_right, textcolor=color.white, xloc=xloc.bar_index, yloc=yloc.price, textalign=text.align_right)

// Alert
alertcondition(buySignal, title = "Buy Alert", message = "SMA and EMA crossed over. Time to Buy!")
alertcondition(sellSignal, title = "Sell Alert", message = "SMA and EMA crossed under. Time to Sell!")
...
var string grp_patterns = "████ ABCD Patterns ████"

step  = input.int(45, title='use x steps' , minval=1, maxval=45, group = grp_patterns)
left  = input.int(3 , minval= 1, title='L', inline='1', group='left, + x for every next \'left\', right = % of left')
lPlus = input.int(2 , minval= 1, title='+', inline='1', group='left, + x for every next \'left\', right = % of left')
perc  = input.int(10, title='R = % of L'  , inline='1', group='left, + x for every next \'left\', right = % of left')
//right     = input(1., title="right", inline="2")

minAC = input.float(0.000, title='min AB -> C')
maxAC = input.float(1.000, title='max AB -> C')
minBD = input.float(1.000, title='min BC -> D')
maxBD = input.float(5.000, title='max BC -> D')

bull = input(true, title='step bullish patterns')
bear = input(true, title='step bearish patterns')

c_labelBull = input.color(color.new(color.lime, 100), title='Bullish label'     , group='colors')
c_lbTxtBull = input.color(color.new(color.lime,  20), title='Bullish label text', group='colors')
c_linesBull = input.color(color.new(color.lime,   0), title='Bullish lines'     , group='colors')

c_labelBear = input.color(color.new(color.red , 100), title='Bearish label'     , group='colors')
c_lbTxtBear = input.color(color.new(color.red ,  20), title='Bearish label text', group='colors')
c_linesBear = input.color(color.new(color.red ,   0), title='Bearish lines'     , group='colors')

canh = input.bool(true, title='Patterns', group=grp_patterns, inline='Canh')

var alertBull = array.new_int(0), array.unshift(alertBull, 0)
var alertBear = array.new_int(0), array.unshift(alertBear, 0)

if array.size(alertBull) > 10
    array.pop(alertBull)
if array.size(alertBear) > 10
    array.pop(alertBear)
...
// var a_colZZ = array.from(color.red, color.orange, color.yellow, color.green, color.blue, color.fuchsia, color.purple, color.maroon, color.lime, color.white, color.aqua, color.olive, color.navy, color.gray, color.red, color.orange, color.yellow, color.green, color.blue, color.fuchsia, color.purple, color.maroon, color.lime, color.white, color.aqua, color.olive, color.navy, color.gray, color.red, color.orange, color.yellow, color.green, color.blue, color.fuchsia, color.purple, color.maroon, color.lime, color.white, color.aqua, color.olive, color.navy, color.gray, color.red, color.orange, color.yellow, color.green, color.blue, color.fuchsia, color.purple, color.maroon, color.lime, color.white, color.aqua, color.olive, color.navy, color.gray)
...
f_draw(a, a_ZZ, a_pv, left) =>
    if a < step
        right = math.round(math.max(1, left * (perc / 100)))
        //
        x1 = 0
        x2 = 0
        y1 = 0.
        ...
        var _xy = 0.
        //
        ph = ta.pivothigh(hi, left, right)
        pl = ta.pivotlow(lo , left, right)
        //
        if barstate.isfirst
            array.unshift(a_ZZ, line.new(bar_index, close, bar_index, close, color=color.new(color.blue, 100)))
            array.unshift(a_pv, -1)
        //
        if array.size(a_ZZ) > 10
            line.delete(array.pop(a_ZZ))
            array.pop(a_pv)
        //
        if ph
            x1 := line.get_x2(array.get(a_ZZ, 0))
            y1 := line.get_y2(array.get(a_ZZ, 0))

            x2 := bar_index - right
            y2 := high       [right]

            if array.get(a_pv, 0) == -1  // if previous point was a pl, add, and change direction (1)
                array.unshift(a_ZZ, line.new(x1, y1, x2, y2, color=color.new(color.blue, 100))) //, color=array.get(a_colZZ, a)))
                array.unshift(a_pv, 1)
            else
                if array.get(a_pv, 0) == 1 and ph > line.get_y2(array.get(a_ZZ, 0))
                    line.set_xy2(array.get(a_ZZ, 0), x2, y2)  // if previous point was ph, check if it is higher
        //
        if pl
            x1 := line.get_x2(array.get(a_ZZ, 0))
            y1 := line.get_y2(array.get(a_ZZ, 0))
            ...
            y2 := low        [right]
            //
            if array.get(a_pv, 0) == 1
                array.unshift(a_ZZ, line.new(x1, y1, x2, y2, color=color.new(color.blue, 100))) //, color=array.get(a_colZZ, a)))
                array.unshift(a_pv, -1)
            else
                if array.get(a_pv, 0) == -1 and pl < line.get_y2(array.get(a_ZZ, 0))
                    line.set_xy2(array.get(a_ZZ, 0), x2, y2)
        //
        if array.size(a_ZZ) > 4
            _dx := line.get_x2(array.get(a_ZZ, 0))
            _dy := line.get_y2(array.get(a_ZZ, 0))
            _cx := line.get_x2(array.get(a_ZZ, 1))
            _cy := line.get_y2(array.get(a_ZZ, 1))
            _bx := line.get_x2(array.get(a_ZZ, 2))
            _by := line.get_y2(array.get(a_ZZ, 2))
            _ax := line.get_x2(array.get(a_ZZ, 3))
            _ay := line.get_y2(array.get(a_ZZ, 3))
            _xx := line.get_x2(array.get(a_ZZ, 4))
            _xy := line.get_y2(array.get(a_ZZ, 4))
            //
            if f_isTrueBear(_xy, _ay, _by, _cy, _dy) and bear and close <= _cy
                //
                if array.size(a_HP) > 12 // if more than 2 patterns in array c must be different than c[1] or c[2] (if 2 pat are very close to each other)
                    if _cx != line.get_x1(array.get(a_HP, 1)) and 
                       _cy != line.get_y1(array.get(a_HP, 1)) and
                       _cx != line.get_x1(array.get(a_HP, 7)) and 
                       _cy != line.get_y1(array.get(a_HP, 7)) 
                        array.unshift(a_HP, line.new (bar_index, low, _dx, _dy, width=1, color=c_linesBear, style=line.style_arrow_left))
                        array.unshift(a_HP, line.new (bar_index, _cy, _cx, _cy, width=1, color=c_linesBear, style=line.style_dotted))
                        array.unshift(a_HP, line.new (_bx      , _by, _dx, _by, width=1, color=c_linesBear, style=line.style_dotted))
                        array.unshift(a_HP, line.new (_dx      , _dy, _cx, _cy, width=2, color=c_linesBear))
                        array.unshift(a_HP, line.new (_cx      , _cy, _bx, _by, width=2, color=c_linesBear))
                        array.unshift(a_HP, line.new (_bx      , _by, _ax, _ay, width=2, color=c_linesBear))
                        array.unshift(a_lb, label.new(_dx      , _dy, text='Canv Short', style=label.style_label_down, color=c_labelBear, textcolor=c_lbTxtBear, size=size.tiny))
                        array.set(alertBear, 0, array.get(alertBear, 0) + 1)
                //
                else if array.size(a_HP) > 6
                    if _cx != line.get_x1(array.get(a_HP, 1)) and 
                       _cy != line.get_y1(array.get(a_HP, 1))  // if more then 2 lines in array (= 2 patterns)
                        array.unshift(a_HP, line.new (bar_index, low, _dx, _dy, width=1, color=c_linesBear, style=line.style_arrow_left))
                        array.unshift(a_HP, line.new (bar_index, _cy, _cx, _cy, width=1, color=c_linesBear, style=line.style_dotted))
                        array.unshift(a_HP, line.new (_bx      , _by, _dx, _by, width=1, color=c_linesBear, style=line.style_dotted))
                        array.unshift(a_HP, line.new (_dx      , _dy, _cx, _cy, width=2, color=c_linesBear))
            ...
            if f_isTrueBull(_xy, _ay, _by, _cy, _dy) and bull and close >= _cy
                ...
                if array.size(a_HP) > 12 // if more than 2 patterns in array c must be different than c[1] or c[2] (if 2 pat are very close to each other)
                    if _cx != line.get_x1(array.get(a_HP, 1)) and 
                       ...
                       _cx != line.get_x1(array.get(a_HP, 7)) and 
                       _cy != line.get_y1(array.get(a_HP, 7)) 
                        array.unshift(a_HP, line.new (bar_index, high, _dx, _dy, width=1, color=c_linesBull, style=line.style_arrow_left))
                        array.unshift(a_HP, line.new (bar_index, _cy , _cx, _cy, width=1, color=c_linesBull, style=line.style_dotted))
                        array.unshift(a_HP, line.new (_bx      , _by , _dx, _by, width=1, color=c_linesBull, style=line.style_dotted))                     
                        array.unshift(a_HP, line.new (_dx      , _dy , _cx, _cy, width=2, color=c_linesBull))
                        array.unshift(a_HP, line.new (_cx      , _cy , _bx, _by, width=2, color=c_linesBull))
                        array.unshift(a_HP, line.new (_bx      , _by , _ax, _ay, width=2, color=c_linesBull))
                        array.unshift(a_lb, label.new(_dx      , _dy , text='Canv Long', style=label.style_label_up, color=c_labelBull, textcolor=c_lbTxtBull, size=size.tiny))
                        array.set(alertBull, 0, array.get(alertBull, 0) + 1)
                //
                else if array.size(a_HP) > 6
                    if _cx != line.get_x1(array.get(a_HP, 1)) and 
                       _cy != line.get_y1(array.get(a_HP, 1))
                        array.unshift(a_HP, line.new (bar_index, high, _dx, _dy, width=1, color=c_linesBull, style=line.style_arrow_left))
                        array.unshift(a_HP, line.new (bar_index, _cy , _cx, _cy, width=1, color=c_linesBull, style=line.style_dotted))
                        array.unshift(a_HP, line.new (_bx      , _by , _dx, _by, width=1, color=c_linesBull, style=line.style_dotted))                     
                        array.unshift(a_HP, line.new (_dx      , _dy , _cx, _cy, width=2, color=c_linesBull))
...
num  = 0
f_draw(num, a_ZZ1, a_pv1, left=left)  //, plot(left), plot(round(max(1, left * (perc / 100))))  
num += 1, left += lPlus
...
num += 1, left += lPlus
f_draw(num, a_ZZ11, a_pv11, left=left)  //, plot(left), plot(round(max(1, left * (perc / 100))))
num += 1, left += lPlus
...
num += 1, left += lPlus
f_draw(num, a_ZZ21, a_pv21, left=left)  //, plot(left), plot(round(max(1, left * (perc / 100))))
num += 1, left += lPlus
...
num += 1, left += lPlus
f_draw(num, a_ZZ31, a_pv31, left=left)  //, plot(left), plot(round(max(1, left * (perc / 100))))
num += 1, left += lPlus
...
num += 1, left += lPlus
f_draw(num, a_ZZ41, a_pv41, left=left)  //, plot(left), plot(round(max(1, left * (perc / 100))))
num += 1, left += lPlus
...
alertcondition(array.get(alertBull, 0) > 0, 'Bullish Canvh', 'Bullish Canvh')
alertcondition(array.get(alertBear, 0) > 0, 'Bearish Canvh', 'Bearish Canvh')
//plot(array.get(alertBear, 0))
//plot(array.get(alertBull, 0))
